<script lang="ts">
  ...
  import SphereGrid from '$lib/components/SphereGrid.svelte';
  import type { Clone } from '$lib/types';

  let { data }: { data: { clones: Clone[] } } = $props();
</script>
...
<div class="relative h-screen w-screen overflow-hidden">
  ...
  <div class="pointer-events-none absolute left-1/2 top-8 z-10 -translate-x-1/2 text-center">
    ...
    <p class="mt-1 text-[10px] lowercase tracking-[0.3em] text-slate-500">
      {data.clones.length} active · {data.clones.reduce((s, c) => s + c.activityCount, 0)} signals
    </p>
  </div>

  <!-- bottom legend -->
  <div class="absolute bottom-8 left-1/2 z-10 -translate-x-1/2">
    <div class="flex items-center gap-6 rounded-full border border-slate-800/60 bg-slate-900/40 px-6 py-3 backdrop-blur-md">
