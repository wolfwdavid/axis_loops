<script lang="ts">
  import type { Pattern, Decision } from './types';
  import { appendDecision } from './storage';
  ...
  function decide(verdict: 'approve' | 'reject', sentTo: ('salesforce' | 'linear' | 'slack')[] = []) {
    const key =
      verdict === 'reject' ? 'reject' : sentTo.includes('salesforce') ? 'approve-sf' : 'approve-linear';
    loading = key;
    ...
    // simulate a small write latency
    setTimeout(() => {
...
>
  ...
  <div class="flex items-start justify-between gap-4">
    <div class="flex-1">
      ...
      <div class="mt-3 flex flex-wrap items-center gap-2">
        ...
        <span class="rounded-full border px-2 py-0.5 text-[9px] uppercase tracking-[0.3em] {severityStyles[pattern.severity]}">
          {pattern.severity}
        </span>
    ...
    <div class="mt-5 grid grid-cols-1 gap-2 sm:grid-cols-2">
      {#each implEntries as [k, v] (k)}
        <div class="rounded-lg border border-slate-800/60 bg-slate-950/30 p-3">
          <p class="text-[9px] uppercase tracking-[0.3em] text-slate-500">{implLabels[k] ?? k}</p>
      ...
      {#each pattern.citations as cite, i (i)}
        <li class="rounded-lg border border-slate-800/60 bg-slate-950/50 p-3">
          <p class="text-[9px] uppercase tracking-[0.3em] text-slate-500">
            {cite.transcriptFile}{cite.speaker ? ` · ${cite.speaker}` : ''}{cite.timestamp ? ` · ${cite.timestamp}` : ''}
          </p>
  ...
  {#if sentLabel}
    <p class="mt-3 text-[10px] uppercase tracking-[0.3em] text-emerald-400">✓ {sentLabel}</p>
  {/if}
</article>
