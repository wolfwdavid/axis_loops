<script lang="ts">
  ...
  import type { Intake, IntakeResponse, SpawnResponse } from '$lib/types';
  ...
  let stage = $state<'idle' | 'profiling' | 'spawning' | 'done' | 'error'>('idle');
  ...
  async function onSubmit(e: SubmitEvent) {
    ...
    try {
      ...
      if (!intakeRes.ok) throw new Error(`intake failed · http ${intakeRes.status}`);
      const intakeData = (await intakeRes.json()) as IntakeResponse;
      suggestedName = intakeData.suggestedName;

      stage = 'spawning';
      // small dramatic pause so the user sees the animation
      ...
      if (!spawnRes.ok) throw new Error(`spawn failed · http ${spawnRes.status}`);
      const spawnData = (await spawnRes.json()) as SpawnResponse;
      ...
      stage = 'error';
      errorMsg = err instanceof Error ? err.message : 'unknown failure';
    }
  }
</script>
...
<form onsubmit={onSubmit} class="space-y-5">
  ...
  {#if errorMsg}
    <p class="text-center text-[10px] uppercase tracking-[0.3em] text-red-400">⚠ {errorMsg}</p>
  {/if}
</form>
